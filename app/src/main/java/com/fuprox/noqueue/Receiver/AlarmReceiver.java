...
//                    Log.e(TAG, "get_booking_notify: active booking loccal db " );
                    ...
                    if (new Dbhelper(context).getnotistate().equals("1")){ //notify if notifications is enabled
                        ...
                        String content="Queue ticket status";
...
//                            infront = 1;
                ...
                e.printStackTrace();
                Log.e(TAG, "run: check infront connection error"+e.toString() );
                error_serv="There was a problem communicating with the servers. \n\n Try again later.";
            }
            return null;
        }
        ...
        protected void onPostExecute(String s) {
            super.onPostExecute(s);
            int stored_q_count = new Dbhelper(context).get_q_no(booking_id);
            Log.e(TAG, "onPostExecute: infront alarm "+ stored_q_count);

            if (error_serv.equals(" ") ){
                if (infront==0 && stored_q_count!=0){
//                    new Dbhelper(context).updateorder(String.valueOf(booking_id),"0");
                    ...
                    update_db_ahead(context,booking_id,infront);

                }
                else if (infront==15 && stored_q_count!=15 ){
                    notify_(context,booking_id,id,content,"15 People ahead of you",branchname,servicename);
                    update_db_ahead(context,booking_id,infront);

                }
                else if (infront==10 && stored_q_count!=10){
                    notify_(context,booking_id,id,content,"10 People ahead of you",branchname,servicename);
                    update_db_ahead(context,booking_id,infront);

                }
                else if (infront <= 5 && infront != 1 && infront!=0 &&  stored_q_count!=infront){
//
                    Log.e(TAG, "onPostExecute: notified for "+booking_id );
                    notify_(context,booking_id,id,content,"Less than 5 People ahead of you",branchname,servicename);
                    update_db_ahead(context,booking_id,infront);
                }
                else if (infront==1 && stored_q_count!=1){
                    Log.e(TAG, "onPostExecute: notified for "+booking_id );
                ...
                else if  (infront==20 && stored_q_count!=20) {
                    Log.e(TAG, "onPostExecute: notified for "+booking_id );
